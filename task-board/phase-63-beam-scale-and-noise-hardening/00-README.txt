...
- Latest accepted retained run: `run-phase63-beam-100k-recall-diagnostic-rules-work-life-challenges-event-order-current-20260615T024400Z`, evidence-chat recall 0.9045964676950593, missed 60/355, wrong-recall/noise 205/400, zero-recall 0.

Current Task Queue
------------------

1. Keep the latest work-life-challenges event-order coverage repair (zero-recall is 0; reducing event_ordering miss/noise one case at a time). Some event_ordering questions match a broad preempting selector (e.g. personalWorkChallenge at route position 4) and need a scoped one-gate suppression so the precise coverage in temporal_order can win. Remaining high-noise event_ordering targets sit at/past the reinforcement-ripple threshold (~29), so further high-noise gains may require reverts. Noise targets up to perturbation ~25 are now validated clean; the reinforcement-ripple regression threshold is above ~25. The uncapped event-order coverage selectors are now iterated from a single table in sourceOrderTemporal.ts (add a new family by appending one entry, keeping the file under the 900-line cap). Pre-screen targets: prefer the lowest reinforcement perturbation (fewest noisy chats to shed) and avoid conversations with a fragile recall-1 temporal_reasoning case, which the diagnostic's per-conversation shared reinforcement store can regress when the event-order target is recovered. Watch for event-order questions that read as aggregates (money/count cues); they need a one-gate aggregateEvidenceQuery suppression so the aggregate route does not preempt the coverage.
