...
- Latest accepted retained run: `run-phase63-beam-100k-recall-diagnostic-rules-patent-webinar-contradiction-current-20260615T032116Z`, evidence-chat recall 0.90741336910351, missed 58/355, wrong-recall/noise 203/400, zero-recall 0.

Current Task Queue
------------------

1. Keep the latest patent-webinar contradiction repair (zero-recall is 0). The event-order coverage workstream is exhausted; the active workstream is now contradiction_resolution, recovered via the contradiction.ts first-statement/denial pair recipe (gate + FIRST_STATEMENT/DENIAL patterns + selector chained in selectContradictionEvidencePair). Remaining contradiction_resolution misses are mostly two-turn pairs at recall 0.5 with one turn missing; after those, temporal_reasoning, multi_session_reasoning, knowledge_update, and instruction_following remain. Noise targets up to perturbation ~25 are now validated clean; the reinforcement-ripple regression threshold is above ~25. The uncapped event-order coverage selectors are now iterated from a single table in sourceOrderTemporal.ts (add a new family by appending one entry, keeping the file under the 900-line cap). Pre-screen targets: prefer the lowest reinforcement perturbation (fewest noisy chats to shed) and avoid conversations with a fragile recall-1 temporal_reasoning case, which the diagnostic's per-conversation shared reinforcement store can regress when the event-order target is recovered. Watch for event-order questions that read as aggregates (money/count cues); they need a one-gate aggregateEvidenceQuery suppression so the aggregate route does not preempt the coverage.
