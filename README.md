...
Phase 35 另外新增了一组 installed-host config 命令：`goodmemory install|uninstall <codex|claude>` 和 `goodmemory enable|disable <codex|claude>`。这组命令现在会管理 GoodMemory 自己的全局 host config、repo-local opt-in block，以及用户级 MCP 注册：Codex 写入 `~/.codex/config.toml` 里的 `mcp_servers.goodmemory`，Claude Code 写入 `~/.claude.json` 里的 `mcpServers.goodmemory`。它们不会隐式创建 canonical memory state，也还没有取代 `goodmemory codex bootstrap` / `goodmemory claude bootstrap` 作为已接受的 host wiring 路径。
