...
Phase 35 另外新增了一组 installed-host config 命令：`goodmemory install|uninstall <codex|claude>` 和 `goodmemory enable|disable <codex|claude>`。这组命令现在会管理 GoodMemory 自己的全局 host config、repo-local opt-in block、用户级 MCP 注册，以及用户级 hook wiring：Codex 会合并 `~/.codex/config.toml` 里的 `mcp_servers.goodmemory` 和 `features.codex_hooks`，再写入 `~/.codex/hooks.json`；Claude Code 会合并 `~/.claude.json` 里的 `mcpServers.goodmemory`，再写入 `~/.claude/settings.json`。它们不会隐式创建 canonical memory state，也还没有取代 `goodmemory codex bootstrap` / `goodmemory claude bootstrap` 作为已接受的 host wiring 路径。

Phase 35 也新增了 machine-facing 的 hook runtime 命令：`goodmemory codex hook <session-start|user-prompt-submit>` 和 `goodmemory claude hook <session-start|user-prompt-submit>`。这些命令从 stdin 读取 host hook JSON，走现有 `recall()` + `buildContext()` 路径，并在 repo 已显式 enable 时把压缩后的 memory context 通过 stdout JSON 返回给 host。`goodmemory install codex|claude` 现在会自动把这些 hook handlers 注册到对应 host 的用户级配置里。
