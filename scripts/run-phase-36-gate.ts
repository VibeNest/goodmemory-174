...
export interface Phase36GateCommandOptions {
...
function buildPhase36DeterministicRegenerateCommand(runId: string): string {
  return `bun run eval:phase-36 --run-id ${runId}`;
}
...
export function resolvePhase36CanonicalDeterministicReportPath(root: string): string {
  return resolvePhase36DeterministicReportPath(
    root,
    PHASE36_CANONICAL_DETERMINISTIC_RUN_ID,
  );
}

function resolvePhase36DeterministicReportPath(
  root: string,
  runId: string,
): string {
  return join(
...
export function buildPhase36GateCommands(
  root: string,
  options: Phase36GateCommandOptions = {},
): Phase36GateCommand[] {
  const deterministicRunId =
    options.deterministicRunId ?? PHASE36_CANONICAL_DETERMINISTIC_RUN_ID;
...
function validatePhase36DeterministicReport(
  ...
  if (!isReportPathUnder(root, reportPath, join(root, "reports/eval/fallback/phase-36"))) {
    throw new Error("Phase 36 deterministic report path is outside the phase output directory.");
  }
  ...
  return {
    ...
    evidence: {
      deterministicReport: {
        ...
        reason: input.reason,
        regenerateCommand: buildPhase36DeterministicRegenerateCommand(input.deterministicRunId),
        status: "blocked",
      },
      liveMemory: {
...
export async function runPhase36QualityGate(
  ...
  const timestamp = now();
  const runId = options.runId ?? buildPhase36GateRunId(timestamp);
  const deterministicRunId =
  ...
  const deterministicReportPath = resolvePhase36DeterministicReportPath(
    root,
    deterministicRunId,
  );
  ...
  for (const command of buildPhase36GateCommands(root, { deterministicRunId })) {
  ...
  const report: Phase36GateReport = {
    ...
    evidence: {
      deterministicReport: {
        ...
        regenerateCommand: buildPhase36DeterministicRegenerateCommand(deterministicRunId),
