...
import { PHASE55_CANONICAL_RUN_ID } from "./run-phase-55-eval";
import {
  PHASE55_CANONICAL_LIVE_RUN_ID,
} from "./run-phase-55-live-memory";
import {
...
export interface Phase55GateOptions {
  liveReportPath?: string;
  outputDir?: string;
  runId?: string;
}

export interface Phase55GateCommand {
  args: string[];
  cwd: string;
  label: string;
}

export interface Phase55GateCommandResult {
  durationMs: number;
  exitCode: number;
  stderr: string;
  stdout: string;
...
export interface Phase55GateExecutionResult {
  command: string;
  durationMs: number;
  exitCode: number;
  label: string;
...
export interface Phase55LiveReportEvidence {
...
export interface Phase55GateReport {
  acceptance: {
    decision: "accepted" | "blocked";
    reason: string;
  };
  commands: Phase55GateExecutionResult[];
  evidence: {
    deterministicReport: {
    ...
    liveMemoryReport: Phase55LiveReportEvidence;
  };
  generatedAt: string;
  generatedBy: "scripts/run-phase-55-gate.ts";
  phase: "phase-55";
  runDirectory: string;
  runId: string;
}

export interface Phase55GateDependencies {
  ensureDir?: (path: string, options?: { recursive?: boolean }) => Promise<void>;
  now?: () => string;
  readTextFile?: (path: string) => Promise<string>;
  runCommand?: (command: Phase55GateCommand) => Promise<Phase55GateCommandResult>;
  writeTextFile?: (path: string, content: string) => Promise<void>;
}

const GENERATED_BY = "scripts/run-phase-55-gate.ts";
export const PHASE55_CANONICAL_GATE_RUN_ID = "run-20260503233000";
const FROZEN_BASELINE_RAW_LEAK_COUNT = 0;
const FROZEN_BASELINE_RAW_PASSED_BLOCKING = 5;
...
async function defaultRunCommand(
  command: Phase55GateCommand,
): Promise<Phase55GateCommandResult> {
...
export function resolvePhase55GateOutputDir(root: string): string {
  return join(root, "reports/quality-gates/phase-55");
}

export function resolvePhase55CanonicalLiveReportPath(root: string): string {
  return join(
    resolvePhase55LiveMemoryOutputDir(root),
    PHASE55_CANONICAL_LIVE_RUN_ID,
    "report.json",
  );
}

export function resolvePhase55CanonicalFallbackReportPath(root: string): string {
  return join(
    resolvePhase55FallbackOutputDir(root),
    PHASE55_CANONICAL_RUN_ID,
    "report.json",
  );
}

export function parsePhase55GateCliOptions(
...
function parseResearchReport(
  ...
  if (!isRecord(parsed)) {
    return "Phase 55 live report must be a JSON object.";
  }
  if (parsed.kind !== "goodmemory") {
    return "Phase 55 gate requires a GoodMemory research report.";
  }
  if (parsed.mode !== "live") {
    return "Phase 55 gate requires a live-mode report.";
  }
  if (!isRecord(parsed.profiles)) {
    return "Phase 55 live report must include profiles.";
  }

  return parsed as unknown as ImplicitMemBenchResearchReport;
}

function buildPhase55GateCommands(root: string): Phase55GateCommand[] {
  return [
    {
    ...
    },
    {
      args: [
        "bun",
        "test",
        "tests/unit/eval.phase55.test.ts",
        ...
        "tests/unit/run-phase-55.script.test.ts",
        "tests/unit/run-phase-55.gate.test.ts",
      ],
      cwd: root,
      label: "phase-55-targeted-regressions",
    },
    {
      args: [
        "bun",
    ...
    },
    {
      args: [
        "bun",
        "run",
        "eval:phase-55-live-memory",
        "--",
        "--run-id",
        PHASE55_CANONICAL_LIVE_RUN_ID,
      ],
      cwd: root,
      label: "eval:phase-55-live-memory",
    },
  ];
}
...
function parseSmokeReport(
  ...
  if (!isRecord(parsed)) {
    return "Phase 55 deterministic report must be a JSON object.";
  }
  if (parsed.kind !== "goodmemory") {
    return "Phase 55 deterministic report must be a GoodMemory research report.";
  }
  if (parsed.mode !== "smoke") {
    return "Phase 55 deterministic report must be a smoke-mode report.";
  }
  if (!isRecord(parsed.profiles)) {
    return "Phase 55 deterministic report must include profiles.";
  }
...
function validatePhase55LiveEvidence(input: {
  canonicalLiveReportPath: string;
  liveReportPath: string;
  report: ImplicitMemBenchResearchReport;
}): Phase55LiveReportEvidence {
...
export async function runPhase55Gate(
  options?: Phase55GateOptions,
  dependencies?: Phase55GateDependencies,
): Promise<Phase55GateReport> {
  const root = resolvePhase55RepoRoot();
  const runId = options?.runId ?? PHASE55_CANONICAL_GATE_RUN_ID;
  const outputDir = resolve(options?.outputDir ?? resolvePhase55GateOutputDir(root));
  ...
  const canonicalFallbackReportPath = resolvePhase55CanonicalFallbackReportPath(root);
  const deterministicEvidence = {
    artifactKind: "ignored_generated" as const,
    ignoredReportPath: toRepoRelativePath(root, canonicalFallbackReportPath),
    reason:
      "Phase 55 deterministic targeted eval is generated evidence and remains reproducible ignored output.",
    regenerateCommand: `bun run eval:phase-55 -- --run-id ${PHASE55_CANONICAL_RUN_ID}`,
    status: "blocked" as const,
  };

  await ensureDir(runDirectory, { recursive: true });
  const commands: Phase55GateExecutionResult[] = [];

  for (const command of buildPhase55GateCommands(root)) {
    ...
    if (result.exitCode !== 0) {
      const report: Phase55GateReport = {
        acceptance: {
          decision: "blocked",
          reason: `Phase 55 gate command failed: ${command.label}`,
        },
        commands,
        evidence: {
          deterministicReport: deterministicEvidence,
          liveMemoryReport: {
            canonicalLiveReportPath: resolvePhase55CanonicalLiveReportPath(root),
            distilledBlockingCases: 0,
            distilledPassedBlockingCases: 0,
            explicitRecallLeakDelta: 0,
            liveReportPath: options?.liveReportPath ?? resolvePhase55CanonicalLiveReportPath(root),
            rawBlockingCases: 0,
      ...
      await writeTextFile(
        join(runDirectory, "phase-55-quality-gate.json"),
  ...
  try {
    ...
    const report: Phase55GateReport = {
      acceptance: {
        decision: "blocked",
        reason: `Phase 55 deterministic report is missing or unreadable: ${error instanceof Error ? error.message : String(error)}`,
      },
      commands,
      evidence: {
        deterministicReport: deterministicEvidence,
  ...
  const canonicalLiveReportPath = resolvePhase55CanonicalLiveReportPath(root);
  ...
  try {
    ...
    const report: Phase55GateReport = {
      acceptance: {
        decision: "blocked",
        reason: `Phase 55 canonical live report is missing or unreadable: ${error instanceof Error ? error.message : String(error)}`,
      },
      commands,
      evidence: {
        deterministicReport: {
    ...
    acceptance: {
      ...
      reason:
        typeof parsedReport === "string"
          ? parsedReport
          : accepted
            ? "Phase 55 targeted deterministic and live raw-carryover evidence passed."
            : "Phase 55 live-memory raw-carryover evidence did not meet the targeted bar.",
    },
    commands,
    evidence: {
      deterministicReport: {
...
async function main(): Promise<void> {
  const report = await runPhase55Gate(parsePhase55GateCliOptions(process.argv));
