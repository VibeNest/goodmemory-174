...
export interface Phase22LiveMemoryOptions extends FixtureEvalOptions {
  runId?: string;
}

export interface Phase22LiveMemoryDependencies {
...
export interface Phase22LiveMemoryCliDependencies {
  argv?: readonly string[];
  log?: (message: string) => void;
  runEval?: (
    options?: Phase22LiveMemoryOptions,
    dependencies?: Phase22LiveMemoryDependencies,
  ) => Promise<Phase22LiveMemoryReport>;
}

export interface Phase22LiveMemoryReport {
  assist: EvalSuiteResult;
  observe: EvalSuiteResult;
  outputDir: string;
}

const PHASE_22_RECALL_ROUTER_STRATEGIES = [
  "llm-assisted",
] as const satisfies RecallRouterStrategy[];

export function resolvePhase22LiveMemoryOutputDir(root: string): string {
  return join(root, "reports/eval/live-memory/phase-22");
}

function resolvePhase22LiveMemoryScenarioIds(explicit?: string[]): string[] {
...
function resolvePhase22BaseRunId(runId?: string): string {
  return runId ?? `run-${Date.now()}`;
}

function resolvePhase22RunId(
  baseRunId: string,
...
async function runPhase22LiveMemoryMode(input: {
  baseRunId: string;
  createEmbeddingAdapter: typeof createProviderEmbeddingAdapter;
  createJudgeModel: typeof createProviderJudgeModel;
  createMemory: typeof createInternalGoodMemory;
  ...
  return input.runSuite({
    ...
    runId: resolvePhase22RunId(input.baseRunId, input.mode),
    limit: input.options?.limit,
    scenarioIds: resolvePhase22LiveMemoryScenarioIds(input.options?.scenarioIds),
    ...
    strategies: input.options?.strategies ?? [...PHASE_22_RECALL_ROUTER_STRATEGIES],
...
export async function runPhase22LiveMemoryEval(
  input?: Phase22LiveMemoryOptions,
  dependencies?: Phase22LiveMemoryDependencies,
): Promise<Phase22LiveMemoryReport> {
  ...
  const outputDir = input?.outputDir ?? resolvePhase22LiveMemoryOutputDir(root);
  const baseRunId = resolvePhase22BaseRunId(input?.runId);

  const observe = await runPhase22LiveMemoryMode({
    baseRunId,
  ...
  const assist = await runPhase22LiveMemoryMode({
    baseRunId,
    createEmbeddingAdapter,
    createJudgeModel,
    createMemory,
...
export function parsePhase22LiveMemoryCliOptions(
  argv: readonly string[],
): Phase22LiveMemoryOptions {
  const args = [...argv];
...
export async function runPhase22LiveMemoryCli(
  dependencies: Phase22LiveMemoryCliDependencies = {},
): Promise<Phase22LiveMemoryReport> {
  const argv = [...(dependencies.argv ?? process.argv)];
  const log = dependencies.log ?? console.log;
  const runEval = dependencies.runEval ?? runPhase22LiveMemoryEval;
  const options = parsePhase22LiveMemoryCliOptions(argv);
