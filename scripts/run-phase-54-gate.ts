...
import { PHASE54_CANONICAL_RUN_ID } from "./run-phase-54-eval";
import {
  PHASE54_CANONICAL_LIVE_RUN_ID,
} from "./run-phase-54-live-memory";
import {
...
export interface Phase54GateOptions {
  liveReportPath?: string;
  outputDir?: string;
  runId?: string;
}

export interface Phase54GateCommand {
  args: string[];
  cwd: string;
  label: string;
}

export interface Phase54GateCommandResult {
  durationMs: number;
  exitCode: number;
  stderr: string;
  stdout: string;
...
export interface Phase54GateExecutionResult {
  command: string;
  durationMs: number;
  exitCode: number;
  label: string;
...
export interface Phase54LiveReportEvidence {
...
export interface Phase54GateReport {
  acceptance: {
    decision: "accepted" | "blocked";
    reason: string;
  };
  commands: Phase54GateExecutionResult[];
  evidence: {
    deterministicReport: {
    ...
    liveMemoryReport: Phase54LiveReportEvidence;
  };
  generatedAt: string;
  generatedBy: "scripts/run-phase-54-gate.ts";
  phase: "phase-54";
  runDirectory: string;
  runId: string;
}

export interface Phase54GateDependencies {
  ensureDir?: (path: string, options?: { recursive?: boolean }) => Promise<void>;
  now?: () => string;
  readTextFile?: (path: string) => Promise<string>;
  runCommand?: (command: Phase54GateCommand) => Promise<Phase54GateCommandResult>;
  writeTextFile?: (path: string, content: string) => Promise<void>;
}

const GENERATED_BY = "scripts/run-phase-54-gate.ts";
export const PHASE54_CANONICAL_GATE_RUN_ID = "run-20260503193000";
const FROZEN_BASELINE_RAW_LEAK_COUNT = 1;
const FROZEN_BASELINE_RAW_PASSED_BLOCKING = 3;
const FROZEN_BASELINE_RAW_STRUCTURED_PASSES = 0;
...
async function defaultRunCommand(
  command: Phase54GateCommand,
): Promise<Phase54GateCommandResult> {
...
export function resolvePhase54GateOutputDir(root: string): string {
  return join(root, "reports/quality-gates/phase-54");
}

export function resolvePhase54CanonicalLiveReportPath(root: string): string {
  return join(
    resolvePhase54LiveMemoryOutputDir(root),
    PHASE54_CANONICAL_LIVE_RUN_ID,
    "report.json",
  );
}

export function resolvePhase54CanonicalFallbackReportPath(root: string): string {
  return join(
    resolvePhase54FallbackOutputDir(root),
    PHASE54_CANONICAL_RUN_ID,
    "report.json",
  );
}

export function parsePhase54GateCliOptions(
...
function parseResearchReport(
  ...
  if (!isRecord(parsed)) {
    return "Phase 54 live report must be a JSON object.";
  }
  if (parsed.kind !== "goodmemory") {
    return "Phase 54 gate requires a GoodMemory research report.";
  }
  if (parsed.mode !== "live") {
    return "Phase 54 gate requires a live-mode report.";
  }
  if (!isRecord(parsed.profiles)) {
    return "Phase 54 live report must include profiles.";
  }

  return parsed as unknown as ImplicitMemBenchResearchReport;
}

function buildPhase54GateCommands(root: string): Phase54GateCommand[] {
  return [
    {
    ...
    },
    {
      args: [
        "bun",
        "test",
        "tests/unit/eval.phase54.test.ts",
        ...
        "tests/unit/run-phase-54.script.test.ts",
        "tests/unit/run-phase-54.gate.test.ts",
      ],
      cwd: root,
      label: "phase-54-targeted-regressions",
    },
    {
      args: [
        "bun",
    ...
    },
    {
      args: [
        "bun",
        "run",
        "eval:phase-54-live-memory",
        "--",
        "--run-id",
        PHASE54_CANONICAL_LIVE_RUN_ID,
      ],
      cwd: root,
      label: "eval:phase-54-live-memory",
    },
  ];
}
...
function parseSmokeReport(
  ...
  if (!isRecord(parsed)) {
    return "Phase 54 deterministic report must be a JSON object.";
  }
  if (parsed.kind !== "goodmemory") {
    return "Phase 54 deterministic report must be a GoodMemory research report.";
  }
  if (parsed.mode !== "smoke") {
    return "Phase 54 deterministic report must be a smoke-mode report.";
  }
  if (!isRecord(parsed.profiles)) {
    return "Phase 54 deterministic report must include profiles.";
  }
...
function validatePhase54LiveEvidence(input: {
  canonicalLiveReportPath: string;
  liveReportPath: string;
  report: ImplicitMemBenchResearchReport;
}): Phase54LiveReportEvidence {
...
export async function runPhase54Gate(
  options?: Phase54GateOptions,
  dependencies?: Phase54GateDependencies,
): Promise<Phase54GateReport> {
  const root = resolvePhase54RepoRoot();
  const runId = options?.runId ?? PHASE54_CANONICAL_GATE_RUN_ID;
  const outputDir = resolve(options?.outputDir ?? resolvePhase54GateOutputDir(root));
  ...
  const canonicalFallbackReportPath = resolvePhase54CanonicalFallbackReportPath(root);
  const deterministicEvidence = {
    artifactKind: "ignored_generated" as const,
    ignoredReportPath: toRepoRelativePath(root, canonicalFallbackReportPath),
    reason:
      "Phase 54 deterministic targeted eval is generated evidence and remains reproducible ignored output.",
    regenerateCommand: `bun run eval:phase-54 -- --run-id ${PHASE54_CANONICAL_RUN_ID}`,
    status: "blocked" as const,
  };

  await ensureDir(runDirectory, { recursive: true });
  const commands: Phase54GateExecutionResult[] = [];

  for (const command of buildPhase54GateCommands(root)) {
    ...
    if (result.exitCode !== 0) {
      const report: Phase54GateReport = {
        acceptance: {
          decision: "blocked",
          reason: `Phase 54 gate command failed: ${command.label}`,
        },
        commands,
        evidence: {
          deterministicReport: deterministicEvidence,
          liveMemoryReport: {
            canonicalLiveReportPath: resolvePhase54CanonicalLiveReportPath(root),
            distilledBlockingCases: 0,
            distilledPassedBlockingCases: 0,
            explicitRecallLeakDelta: 0,
            liveReportPath: options?.liveReportPath ?? resolvePhase54CanonicalLiveReportPath(root),
            rawBlockingCases: 0,
      ...
      await writeTextFile(
        join(runDirectory, "phase-54-quality-gate.json"),
  ...
  try {
    ...
    const report: Phase54GateReport = {
      acceptance: {
        decision: "blocked",
        reason: `Phase 54 deterministic report is missing or unreadable: ${error instanceof Error ? error.message : String(error)}`,
      },
      commands,
      evidence: {
        deterministicReport: deterministicEvidence,
  ...
  const canonicalLiveReportPath = resolvePhase54CanonicalLiveReportPath(root);
  ...
  try {
    ...
    const report: Phase54GateReport = {
      acceptance: {
        decision: "blocked",
        reason: `Phase 54 canonical live report is missing or unreadable: ${error instanceof Error ? error.message : String(error)}`,
      },
      commands,
      evidence: {
        deterministicReport: {
    ...
    acceptance: {
      ...
      reason:
        typeof parsedReport === "string"
          ? parsedReport
          : accepted
            ? "Phase 54 targeted deterministic and live raw-carryover evidence passed."
            : "Phase 54 live-memory raw-carryover evidence did not meet the targeted bar.",
    },
    commands,
    evidence: {
      deterministicReport: {
...
async function main(): Promise<void> {
  const report = await runPhase54Gate(parsePhase54GateCliOptions(process.argv));
