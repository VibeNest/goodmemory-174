...
import {
  persistC3ProjectionVerification,
  verifyC3Projection,
} from "./codex-coding-effect/c3-verifier";

const options = parseOptions(process.argv.slice(2));
const verification = await verifyC3Projection({
  projectionDirectory: options.projectionDirectory,
});
await persistC3ProjectionVerification({
  path: options.outputPath ??
    join(options.projectionDirectory, "c3-verification.json"),
  verification,
});
