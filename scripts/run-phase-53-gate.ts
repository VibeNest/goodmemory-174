...
import { PHASE53_CANONICAL_RUN_ID } from "./run-phase-53-eval";
import {
  PHASE53_CANONICAL_LIVE_RUN_ID,
} from "./run-phase-53-live-memory";
import {
...
export interface Phase53GateOptions {
  liveReportPath?: string;
  outputDir?: string;
  runId?: string;
}

export interface Phase53GateCommand {
  args: string[];
  cwd: string;
  label: string;
}

export interface Phase53GateCommandResult {
  durationMs: number;
  exitCode: number;
  stderr: string;
  stdout: string;
...
export interface Phase53GateExecutionResult {
  command: string;
  durationMs: number;
  exitCode: number;
  label: string;
...
export interface Phase53LiveReportEvidence {
...
export interface Phase53GateReport {
  acceptance: {
    decision: "accepted" | "blocked";
    reason: string;
  };
  commands: Phase53GateExecutionResult[];
  evidence: {
    deterministicReport: {
    ...
    liveMemoryReport: Phase53LiveReportEvidence;
  };
  generatedAt: string;
  generatedBy: "scripts/run-phase-53-gate.ts";
  phase: "phase-53";
  runDirectory: string;
  runId: string;
}

export interface Phase53GateDependencies {
  ensureDir?: (path: string, options?: { recursive?: boolean }) => Promise<void>;
  now?: () => string;
  readTextFile?: (path: string) => Promise<string>;
  runCommand?: (command: Phase53GateCommand) => Promise<Phase53GateCommandResult>;
  writeTextFile?: (path: string, content: string) => Promise<void>;
}

const GENERATED_BY = "scripts/run-phase-53-gate.ts";
export const PHASE53_CANONICAL_GATE_RUN_ID = "run-20260502203000";
...
async function defaultRunCommand(
  command: Phase53GateCommand,
): Promise<Phase53GateCommandResult> {
...
export function resolvePhase53GateOutputDir(root: string): string {
  return join(root, "reports/quality-gates/phase-53");
}

export function resolvePhase53CanonicalLiveReportPath(root: string): string {
  return join(
    resolvePhase53LiveMemoryOutputDir(root),
    PHASE53_CANONICAL_LIVE_RUN_ID,
    "report.json",
  );
}

export function resolvePhase53CanonicalFallbackReportPath(root: string): string {
  return join(
    resolvePhase53FallbackOutputDir(root),
    PHASE53_CANONICAL_RUN_ID,
    "report.json",
  );
}

export function parsePhase53GateCliOptions(
...
function parseResearchReport(
  ...
  if (!isRecord(parsed)) {
    return "Phase 53 live report must be a JSON object.";
  }
  if (parsed.kind !== "goodmemory") {
    return "Phase 53 gate requires a GoodMemory research report.";
  }
  if (parsed.mode !== "live") {
    return "Phase 53 gate requires a live-mode report.";
  }
  if (!isRecord(parsed.profiles)) {
    return "Phase 53 live report must include profiles.";
  }

  return parsed as unknown as ImplicitMemBenchResearchReport;
}

function buildPhase53GateCommands(root: string): Phase53GateCommand[] {
  return [
    {
    ...
    },
    {
      args: [
        "bun",
        "test",
        "tests/unit/eval.phase53.test.ts",
        ...
        "tests/unit/run-phase-53.script.test.ts",
        "tests/unit/run-phase-53.gate.test.ts",
      ],
      cwd: root,
      label: "phase-53-targeted-regressions",
    },
    {
      args: [
        "bun",
    ...
    },
    {
      args: [
        "bun",
        "run",
        "eval:phase-53-live-memory",
        "--",
        "--run-id",
        PHASE53_CANONICAL_LIVE_RUN_ID,
      ],
      cwd: root,
      label: "eval:phase-53-live-memory",
    },
  ];
}
...
function parseSmokeReport(
  ...
  if (!isRecord(parsed)) {
    return "Phase 53 deterministic report must be a JSON object.";
  }
  if (parsed.kind !== "goodmemory") {
    return "Phase 53 deterministic report must be a GoodMemory research report.";
  }
  if (parsed.mode !== "smoke") {
    return "Phase 53 deterministic report must be a smoke-mode report.";
  }
  if (!isRecord(parsed.profiles)) {
    return "Phase 53 deterministic report must include profiles.";
  }
...
function validatePhase53LiveEvidence(input: {
  canonicalLiveReportPath: string;
  liveReportPath: string;
  report: ImplicitMemBenchResearchReport;
}): Phase53LiveReportEvidence {
...
export async function runPhase53Gate(
  options?: Phase53GateOptions,
  dependencies?: Phase53GateDependencies,
): Promise<Phase53GateReport> {
  const root = resolvePhase53RepoRoot();
  const runId = options?.runId ?? PHASE53_CANONICAL_GATE_RUN_ID;
  const outputDir = resolve(options?.outputDir ?? resolvePhase53GateOutputDir(root));
  ...
  const canonicalFallbackReportPath = resolvePhase53CanonicalFallbackReportPath(root);
  const deterministicEvidence = {
    artifactKind: "ignored_generated" as const,
    ignoredReportPath: toRepoRelativePath(root, canonicalFallbackReportPath),
    reason:
      "Phase 53 deterministic targeted eval is generated evidence and remains reproducible ignored output.",
    regenerateCommand: `bun run eval:phase-53 -- --run-id ${PHASE53_CANONICAL_RUN_ID}`,
    status: "blocked" as const,
  };

  await ensureDir(runDirectory, { recursive: true });
  const commands: Phase53GateExecutionResult[] = [];

  for (const command of buildPhase53GateCommands(root)) {
    ...
    if (result.exitCode !== 0) {
      const report: Phase53GateReport = {
        acceptance: {
          decision: "blocked",
          reason: `Phase 53 gate command failed: ${command.label}`,
        },
        commands,
        evidence: {
          deterministicReport: deterministicEvidence,
          liveMemoryReport: {
            canonicalLiveReportPath: resolvePhase53CanonicalLiveReportPath(root),
            distilledBlockingCases: 0,
            distilledPassedBlockingCases: 0,
            explicitRecallLeakDelta: 0,
            liveReportPath: options?.liveReportPath ?? resolvePhase53CanonicalLiveReportPath(root),
            rawBlockingCases: 0,
      ...
      await writeTextFile(
        join(runDirectory, "phase-53-quality-gate.json"),
  ...
  try {
    ...
    const report: Phase53GateReport = {
      acceptance: {
        decision: "blocked",
        reason: `Phase 53 deterministic report is missing or unreadable: ${error instanceof Error ? error.message : String(error)}`,
      },
      commands,
      evidence: {
        deterministicReport: deterministicEvidence,
  ...
  const canonicalLiveReportPath = resolvePhase53CanonicalLiveReportPath(root);
  ...
  try {
    ...
    const report: Phase53GateReport = {
      acceptance: {
        decision: "blocked",
        reason: `Phase 53 canonical live report is missing or unreadable: ${error instanceof Error ? error.message : String(error)}`,
      },
      commands,
      evidence: {
        deterministicReport: {
    ...
    acceptance: {
      ...
      reason:
        typeof parsedReport === "string"
          ? parsedReport
          : accepted
            ? "Phase 53 targeted deterministic and live behavioral evidence passed."
            : "Phase 53 live-memory behavioral evidence did not meet the targeted bar.",
    },
    commands,
    evidence: {
      deterministicReport: {
...
async function main(): Promise<void> {
  const report = await runPhase53Gate(parsePhase53GateCliOptions(process.argv));
