...
- Phase 63 BEAM has an accepted rules-only live closure: routing the answer context through the general evidence pack (`--evidence-pack`, `src/answer/evidencePack.ts`) raised answer accuracy from 0.56 to 0.6525 (261/400) at identical recall (0.9621), `executionFailures: 0`, gate accepted; it remains scenario-fitted and recall-limited, with recall/noise plus answer-quality hardening ongoing.
...
- Accepted BEAM live closure (rules-only, evidence pack): `run-phase63-beam-100k-live-closure-gpt55-evidence-pack-current` — 261/400 answer accuracy (0.6525), evidence-chat recall 0.9620612564274538, missed-recall 20/355, wrong-answer 139/400, `executionFailures: 0`; prerequisite zero-failure diagnostic `run-phase63-beam-100k-recall-diagnostic-rules-postmerge-current`, gate `run-phase63-beam-closure-gate-gpt55-evidence-pack-current` accepted (20/20). Models: `gpt-5.5` answer + `gpt-5.5` judge (`ai.gurkiai.com`). The answer context is built by the general `src/answer/evidencePack.ts` (operation inferred from the question, source-ordered + timestamped, current-value framing): +37 cases over the prior no-pack baseline (`run-phase63-beam-100k-live-closure-gpt55-current`, 224/400 = 0.56, same recall), matching the full-400 ablation (retrieved-evidence-pack 265/400 = 0.6625) and validated cross-benchmark on MemoryAgentBench CR. The 2-hour live closure is resumable (`--resume`, append-only `live-slice-progress.jsonl`) after the first attempt lost a run to a mid-run endpoint TLS outage. Internal measured close evidence only (scenario-fitted recall ~0.96 vs ~0.68 non-gated floor; same-model judge), NOT a public benchmark claim. The closure path supports `goodmemory-rules-only` and `goodmemory-hybrid`.
- MemoryAgentBench (Phase 64) is active. It has deterministic retrieval smokes, external-root rules reports, and a real live-answer path behind `eval:phase-64-smoke -- --live --evidence-pack`; answer scoring is deterministic via upstream match modes, so there is no LLM judge layer for AR/CR/LRU/TTL. Existing synthetic live evidence `run-phase64-mab-synthetic-live-pack` exposed a reasoning-wrapper false negative in LRU scoring (`<think>...</think>\nCarol`); scoring now strips that wrapper before exact-match while preserving the raw generated answer. This is internal synthetic/small-slice evidence, not a public benchmark claim or accepted external-root closure.
- LoCoMo (Phase 65) is brought up as a retrieval-only smoke adapter (`eval:phase-65-smoke`) with deterministic `executionFailures: 0`; its real live LLM answer/judge layer remains deferred. No upstream data is vendored (MemoryAgentBench MIT; LoCoMo CC BY-NC 4.0, non-commercial). First LoCoMo synthetic smoke: evidence recall 1.0 across all five QA categories, multi_hop cross-session chain ready.
