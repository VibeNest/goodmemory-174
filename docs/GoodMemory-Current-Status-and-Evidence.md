...
- Phase 35 has started widening the installed CLI with managed host-config commands: `goodmemory install|uninstall <codex|claude>` and `goodmemory enable|disable <codex|claude>`. At the current slice, those commands manage GoodMemory-owned global config plus repo-local opt-in blocks; they do not yet replace the accepted bootstrap path or claim final hook/MCP registration.
- Phase 35 now also exposes explicit installed hook handlers on the CLI: `goodmemory codex hook <session-start|user-prompt-submit>` and `goodmemory claude hook <session-start|user-prompt-submit>`. These commands are machine-facing, read hook JSON from stdin, and call the existing `recall()` + `buildContext()` path with fail-open behavior when config is missing, disabled, malformed, or empty. User-level automatic hook registration is still pending.
