...
- MemoryAgentBench (Phase 64) and LoCoMo (Phase 65) are brought up as retrieval-only smoke adapters (`eval:phase-64-smoke`, `eval:phase-65-smoke`) mirroring the BEAM recall-diagnostic seam; both are deterministic with `executionFailures: 0` and defer their live LLM answer/judge layers. No upstream data is vendored (MemoryAgentBench MIT; LoCoMo CC BY-NC 4.0, non-commercial). First LoCoMo synthetic smoke: evidence recall 1.0 across all five QA categories, multi_hop cross-session chain ready.
