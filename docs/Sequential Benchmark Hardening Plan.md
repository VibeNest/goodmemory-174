...
- Latest accepted retained diagnostic: `run-phase63-beam-100k-recall-diagnostic-rules-api-key-obtained-contradiction-current-20260615T160746Z`. Thirteenth contradiction_resolution recovery, fourth via the multi-facet recipe. Compared with the workshop-attendance baseline (20260615T154052Z), it raises evidence-chat recall to 0.9499485803711157, lowers missed-recall cases to 28/355 and wrong-recall/noise to 175/400 (zero-recall stays 0), and moves global hit/missing/noise ids from 1005/89/830 to 1007/87/829. Target `2:contradiction_resolution:1` ("Have I obtained an API key for this project?") goes from recall 0.5 to full recall: the affirmative spans three weather-app API turns (32 building an API-rate-limit call tracker, 34 and 36 short follow-up questions about rapid consecutive calls and retrying after the limit) opposed by a denial (70 never actually obtained an API key for the project). It recovers the two missing short follow-up turns 34/36 and returns all four source-ordered [32,34,36,70], shedding one noisy chat (156). Added as one MULTI_FACET_CONTRADICTION_GROUPS entry (`contradiction.apiKeyObtained` = "obtained an API key" && "project", 1/400); the short follow-up facets key on their distinctive question phrasing ("rapid consecutive calls"; "keeps retrying after hitting the rate limit"). Cleanest possible pass: exactly one case delta, zero ripples; turns 34/36 are short and markerless yet recovered — confirming the multi-facet filter handles even brief follow-up turns. One multi-evidence contradiction case remains (5:cr:2), via the same recipe. Remaining partial-recall families: contradiction_resolution (1, multi-evidence), multi_session_reasoning (11, "how many" aggregates have an upstream candidate-pool recall gap), instruction_following (6, answer_ai_question-blocked), and knowledge_update (3, answer_ai_question-blocked). The prior accepted recall change was the twelfth contradiction_resolution recovery (18:contradiction_resolution:1, workshop-attendance, the first five-turn group), which left recall at 0.9485401296668904; the multi-facet recipe (contradictionRules/multiFacetGroups.ts) has now recovered four multi-evidence contradiction cases (passes 89/90/91/92).
