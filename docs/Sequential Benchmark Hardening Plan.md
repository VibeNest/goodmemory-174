...
- Latest accepted retained diagnostic: `run-phase63-beam-100k-recall-diagnostic-rules-snack-budget-update-current-20260615T045305Z`. Third knowledge_update pass. Compared with `run-phase63-beam-100k-recall-diagnostic-rules-probability-study-hours-update-current-20260615T043526Z`, it raises evidence-chat recall to 0.9144556226246368, lowers missed-recall cases to 53/355 and wrong-recall/noise to 198/400 (zero-recall stays 0), and moves global hit/missing/noise ids from 977/117/905 to 978/116/903. Target `14:knowledge_update:1` ("What is my snack budget for ordering themed treats for the movie marathon?", answer $75) goes from recall 0.5 to full recall: it recovers the missing original turn 66 ("increased my snack budget to $65 ... themed cupcakes") to pair with the already-hit updated turn 102 ("adjusted the snack budget to $75 ... themed drinks"), and sheds its two noisy chats (60/222). The repair adds a `snackBudget` update-series module (gate `updateSeries.snackBudget` requiring "snack budget" && "themed treats" + ORIGINAL/UPDATED patterns + `selectSourceOrderedSnackBudgetEvidence`) and one entry to the `UPDATE_SERIES_RULE_SELECTORS` table, modeled on the holiday-gift-budget module. Cleanest possible pass: exactly one case delta, zero ripples, zero newly-missing, zero new noise. Three knowledge_update cases now recovered; the contradiction-pair recipe remains available (three recovered, ten remain) once `contradiction.ts` gets its own table extraction. The remaining partial-recall families are temporal_reasoning, multi_session_reasoning, knowledge_update, instruction_following, and contradiction_resolution.
