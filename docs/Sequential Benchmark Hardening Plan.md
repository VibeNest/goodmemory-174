...
- Latest accepted retained diagnostic: `run-phase63-beam-100k-recall-diagnostic-rules-academic-mentorship-event-order-current-20260614T235658Z`. Compared with `run-phase63-beam-100k-recall-diagnostic-rules-patent-process-stages-event-order-current-20260614T230256Z`, it raises evidence-chat recall to 0.8903331097697295, lowers missed-recall cases to 70/355 and wrong-recall/noise to 213/400 (zero-recall stays 0), and moves global hit/missing/noise ids from 938/156/1029 to 942/152/1009 — a 20-id net noise drop. Target `7:event_ordering:1` ("order in which I brought up different aspects of my academic work and mentorship ... five items") goes from recall 0.2 to full recall: it recovers the four missing designated turns 14/64/170/214 (124 was already a hit) and sheds all 20 noisy chats with zero new noise. The repair adds an `academicMentorshipEventOrder` coverage selector (five facets — meeting the mentor turn 14, essay inspiration turn 64, warrants feedback turn 124, conference-paper debate turn 170, high-grade follow-up turn 214) chained in `sourceOrderTemporal.ts` with an `academicMentorshipEventOrderPlanActive` flag OR'd into `sourceOrderedNamedEntityEventPlanActive`; because the mentor is a disallowed fixture name, every facet keys on the surrounding date/venue/essay phrasing instead. Two accepted recall-neutral same-conversation noise swaps (`7:information_extraction:1`, `7:multi_session_reasoning:2`, each net-zero noise) leave recall unchanged on those cases. It is the seventh event-order coverage selector. Recipe note: this target was pre-screened — two prior high-noise candidates (conv-19, conv-9) were built and reverted because cleanly recovering the event-order target deterministically regressed a same-conversation `temporal_reasoning` case (the diagnostic recalls a conversation's questions against one shared, reinforcement-mutating store); conv-7 was chosen because both its temporal_reasoning cases were already partial, so there was no fragile recall-1 temporal case to perturb.
