...
- LoCoMo (Phase 65) is banked as adapter/live-path/prep complete plus retrieval-boundary evidence, not performance closure. Representative conv-1 live run: 199 questions, answer accuracy 0.020, `executionFailures` 0. Follow-up eval-only retrieval research ruled out dialog windows, rules-light query expansion, and LLM turn-captioning (commit `2b2ec71`); the current lexical/rules retrieval substrate cannot bridge the question-dialog phrasing gap (exact gold-turn recall ~0.07-0.08, zero-retrieval ~0.92). Reopen only as P65-R003, a neural semantic retrieval probe, when a real embedding endpoint is available. No upstream MemoryAgentBench (MIT) or LoCoMo (CC BY-NC 4.0, non-commercial) data is vendored -- real data flows only through the external-root convention.
