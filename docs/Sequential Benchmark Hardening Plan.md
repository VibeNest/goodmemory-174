...
- Latest accepted retained diagnostic: `run-phase63-beam-100k-recall-diagnostic-rules-mentor-interactions-event-order-current-20260615T001300Z`. Compared with `run-phase63-beam-100k-recall-diagnostic-rules-academic-mentorship-event-order-current-20260614T235658Z`, it raises evidence-chat recall to 0.8917415604739548, lowers missed-recall cases to 69/355 and wrong-recall/noise to 212/400 (zero-recall stays 0), and moves global hit/missing/noise ids from 942/152/1009 to 945/149/1006 — a 3-id net noise drop. Target `18:event_ordering:1` ("order in which I brought up different aspects of my interactions with [mentor] ... six items") goes from recall 0.5 to full recall: it recovers the three missing designated turns 198/254/258 (30/138/196 were already hits) and sheds all 3 noisy chats with zero new noise. The repair adds a `mentorInteractionsEventOrder` coverage selector (six facets — suggested workshop turn 30, relaxation technique turn 138, interview-tips meeting turn 196, stress-management follow-up turn 198, leadership advice on the new role turn 254, implementing that advice turn 258) chained in `sourceOrderTemporal.ts` with a `mentorInteractionsEventOrderPlanActive` flag OR'd into `sourceOrderedNamedEntityEventPlanActive`; because the mentor is a disallowed fixture name, the gate keys on the generic "interactions ... six items" phrasing and every facet keys on the surrounding date/venue/technique detail. This pass is the cleanest possible: exactly one case delta, zero ripples, no hit-loss, no newly-missing evidence. It is the eighth event-order coverage selector. Target selection used the validated pre-screen: conv-18 was picked for the lowest reinforcement perturbation (only three noisy chats to shed), so its one fragile recall-1 temporal_reasoning case was left untouched — unlike the reverted high-noise conv-19 and conv-9 attempts.
