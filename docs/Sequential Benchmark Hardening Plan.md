...
- Latest accepted retained diagnostic: `run-phase63-beam-100k-recall-diagnostic-rules-personal-statement-scholarship-interval-current-20260615T121837Z`. Tenth temporal_reasoning interval recovery. Compared with `run-phase63-beam-100k-recall-diagnostic-rules-ai-hiring-webinar-interval-current-20260615T120630Z`, it raises evidence-chat recall to 0.935582383188017, lowers missed-recall cases to 38/355 and wrong-recall/noise to 183/400 (zero-recall stays 0), and moves global hit/missing/noise ids from 992/102/858 to 993/101/854. Target `9:temporal_reasoning:1` ("How many days do I have between finishing my personal statement and the scholarship deadline?", answer 25 days) goes from recall 0.5 to full recall: it recovers the missing start-anchor turn 10 ("complete this multi-purpose personal statement ... for academic, visa, and grant applications ... finish it by April 20, 2024") to pair with the already-hit end-anchor turn 12 ("scholarship deadline on May 15, 2024, and the visa application due June 1, 2024"), and sheds its four noisy chats (48/49/109/185). The repair adds a `personalStatementScholarshipDays` interval case to `sourceOrderTemporalInterval.ts` (gate `temporalInterval.personalStatementScholarshipDays` = "personal statement" && "scholarship deadline", 1/400; START keys on "academic, visa, and grant applications" to exclude the other April-20 personal-statement turns 8/56/78/82; END/four ternary-chain edits). Cleanest possible pass: exactly one case delta, zero ripples — notably conv-9 has a known poison history (a prior high-perturbation 9:event_ordering:1 recovery regressed 9:temporal_reasoning:2), yet this low-perturbation interval recovery left all 20 conv-9 cases unchanged and 9:tr:2 held at recall 1, confirming the conv-9 poison is perturbation-magnitude-dependent. Ten temporal_reasoning cases now recovered; the clean source-marked tr-interval workstream is exhausted. The remaining partial-recall families are temporal_reasoning (2 — 13:tr:1 and 18:tr:1, both lacking the passed/between/ago connector so they do not route through the broad interval gate and need broad-gate widening), multi_session_reasoning (11), contradiction_resolution (9, but contradiction.ts is 897/900 so the next contradiction case needs per-case module extraction first), instruction_following (6, all blocked by answer_ai_question companion turns), and knowledge_update (3, all answer_ai_question). Next loop should PIVOT to multi_session_reasoning (reasoning-bridge recipe) or the contradiction per-case-module extraction.
