...
- Latest accepted retained diagnostic: `run-phase63-beam-100k-recall-diagnostic-rules-creative-collaborations-event-order-current-20260615T012607Z`. Compared with `run-phase63-beam-100k-recall-diagnostic-rules-project-development-event-order-current-20260615T011155Z`, it raises evidence-chat recall to 0.8988777107086966, lowers missed-recall cases to 64/355 and wrong-recall/noise to 208/400 (zero-recall stays 0), and moves global hit/missing/noise ids from 952/142/977 to 956/138/958 — a 19-id net noise drop. Target `17:event_ordering:2` ("order in which I brought up different aspects of my creative collaborations and related plans ... six items") goes from recall 0.5 to full recall: it recovers the three missing designated turns 22/210/258 (110/160/256 were already hits) and sheds all 18 noisy chats with zero new noise. The repair adds a `creativeCollaborationsEventOrder` coverage selector (six facets — film-festival friend turn 22, weekend retreat turn 110, storyboard collaboration turn 160, virtual brainstorming session turn 210, workshop with local artists turn 256, backup-date follow-up turn 258) chained in `sourceOrderTemporal.ts` with a `creativeCollaborationsEventOrderPlanActive` flag OR'd into `sourceOrderedNamedEntityEventPlanActive`; the brainstorming facet keys on the session length and date rather than the named collaborator (a disallowed fixture name). Two positive case deltas, no negatives: as a side effect, surfacing the brainstorming turn also satisfied a designated evidence turn of `17:knowledge_update:2`, lifting it from recall 0.5 to 1. It is the twelfth event-order coverage selector; conv-17's one fragile recall-1 temporal_reasoning case was left untouched (perturbation ~21, within the validated medium tier).
