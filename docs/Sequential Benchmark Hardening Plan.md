...
- Latest accepted retained diagnostic: `run-phase63-beam-100k-recall-diagnostic-rules-prototype-budget-update-current-20260615T050733Z`. Fourth knowledge_update pass. Compared with `run-phase63-beam-100k-recall-diagnostic-rules-snack-budget-update-current-20260615T045305Z`, it raises evidence-chat recall to 0.9158640733288621, lowers missed-recall cases to 52/355 and wrong-recall/noise to 197/400 (zero-recall stays 0), and moves global hit/missing/noise ids from 978/116/903 to 979/115/901. Target `20:knowledge_update:2` ("What is the budget allocated for prototype refinement and patent attorney fees through August?", answer $8,000) goes from recall 0.5 to full recall: it recovers the missing updated turn 176 ("increased budget allocation to $8,000 ... prototype enhancements") to pair with the already-hit original turn 130 ("$7,000 budget for prototype refinement and patent attorney fees"), and sheds its two noisy chats (64/318). The repair adds a `prototypeBudget` update-series module (gate `updateSeries.prototypeBudget` requiring "prototype refinement" && "patent attorney fees" + ORIGINAL/UPDATED patterns + `selectSourceOrderedPrototypeBudgetEvidence`) and one entry to the `UPDATE_SERIES_RULE_SELECTORS` table, modeled on the holiday-gift-budget module. Cleanest possible pass: exactly one case delta, zero ripples, zero newly-missing, zero new noise — and notably conv-20 carries BOTH temporal_reasoning cases at recall 1 yet neither rippled, confirming the update_evidence route's tight pair return is ripple-free even in a double-recall-1-temporal conversation (the prior "AVOID both-tr-recall-1" heuristic was an event_ordering-specific concern about high-noise reinforcement, not knowledge_update). Four knowledge_update cases now recovered; the remaining knowledge_update misses (8/11/16:ku) hinge on `answer_ai_question` turns that carry no `->->` source marker and so fall outside the source-ordered update-series recipe. The contradiction-pair recipe remains available (three recovered, ten remain) once `contradiction.ts` gets its own table extraction. The remaining partial-recall families are temporal_reasoning, multi_session_reasoning, knowledge_update, instruction_following, and contradiction_resolution.
