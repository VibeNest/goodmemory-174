...
- Latest accepted retained diagnostic: `run-phase63-beam-100k-recall-diagnostic-rules-combinatorics-probability-event-order-current-20260613T053232Z`. Compared with `run-phase63-beam-100k-recall-diagnostic-rules-patent-funding-event-order-current-20260613T052049Z`, it raises evidence-chat recall to 0.8852626872345183, lowers missed-recall cases to 73/355 and wrong-recall/noise to 216/400 (zero-recall stays 0), and moves global hit/missing/noise ids from 931/163/1105 to 933/161/1080 — a 25-id net noise drop with two positive case deltas and no regressions. Target `5:event_ordering:2` ("order in which I brought up aspects of combinatorial calculations and probability concepts ... five items") goes from recall 0.5 to full recall: it recovers the one missing designated turn 76 (turn 28 was already a hit) and sheds all 24 noisy chats. The repair adds a `combinatoricsProbabilityEventOrder` coverage selector (two facets — permutations/combinations turn 28, probability-of-two-aces turn 76) chained in `sourceOrderTemporal.ts` with a `combinatoricsProbabilityEventOrderPlanActive` flag OR'd into `sourceOrderedNamedEntityEventPlanActive`. The gate is distinguished from the existing six-item probabilityConcepts coverage by the "combinatorial calculations" / "five items" phrasing. A second case improved as a side effect: `5:multi_session_reasoning:1` gains recall (+0.33) because surfacing turn 76 also satisfies one of its designated evidence turns.
