...
- Latest accepted retained diagnostic: `run-phase63-beam-100k-recall-diagnostic-rules-first-draft-essay-grade-interval-current-20260615T061734Z`. First temporal_reasoning recovery — pivot to the highest-ripple-risk family. Compared with `run-phase63-beam-100k-recall-diagnostic-rules-bootstrap-components-contradiction-current-20260615T055850Z`, it raises evidence-chat recall to 0.9229063268499889, lowers missed-recall cases to 47/355 and wrong-recall/noise to 192/400 (zero-recall stays 0), and moves global hit/missing/noise ids from 983/111/891 to 984/110/888. Target `7:temporal_reasoning:1` ("How many days do I have between finishing my first draft and my goal to improve my essay grades?", answer 31 days) goes from recall 0.5 to full recall: it recovers the missing end-anchor turn 24 ("improve my essay grades from B- to A by June 15") to pair with the already-hit start-anchor turn 116 ("first draft of the essay by May 15"), and sheds its three noisy chats (160/66/192). The repair adds a `firstDraftEssayGradeDays` interval case to `sourceOrderTemporalInterval.ts` (gate `temporalInterval.firstDraftEssayGradeDays` = "first draft" && "improve my essay grades", 1/400; START/END patterns plus the four ternary-chain edits the existing recipe uses; the start pattern keys on "of the essay by May 15" so it does not match the near-duplicate noise turn 66 which says "first draft due May 15"). The broad `isTemporalIntervalQuery` gate routes the question; the new per-case gate selects the exact START/END anchor pair. SIGNIFICANT: temporal_reasoning is the family that defines the reinforcement-ripple mechanism (the reverted passes 50/50-retry were tr regressions), yet this low-perturbation interval recovery left every other temporal_reasoning case unchanged — exactly one case delta, zero ripples (conv-7 carries both tr cases at recall 0.5; the sibling 7:tr:2 held). This establishes that the interval recipe's tight two-anchor return is ripple-free, unlocking the 12-case temporal_reasoning workstream. The remaining partial-recall families are temporal_reasoning (11), multi_session_reasoning (11), contradiction_resolution (9, but contradiction.ts is 897/900 so the next contradiction case needs per-case module extraction first), instruction_following (6, all blocked by answer_ai_question companion turns), and knowledge_update (3, all answer_ai_question).
