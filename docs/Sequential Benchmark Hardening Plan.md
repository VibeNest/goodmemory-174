...
- Latest accepted retained diagnostic: `run-phase63-beam-100k-recall-diagnostic-rules-non-provisional-filing-instruction-current-20260613T043954Z`. Compared with `run-phase63-beam-100k-recall-diagnostic-rules-patent-timelines-instruction-current-20260613T042720Z`, it raises evidence-chat recall to 0.8764363961547059, lowers missed-recall cases to 77/355, wrong-recall/noise to 220/400, and **zero-recall to 0** (every probing question now has nonzero evidence recall), and moves global hit/missing/noise ids from 916/178/1192 to 918/176/1187. Target `20:instruction_following:2` goes from zero recall to full recall, returning both designated turns 180/182 while removing all five noisy chats 302/300/79/164/77, with zero new noise and exactly one case delta. This is a two-turn hybrid (standing instruction 180 + a confirmation turn 182 that restates the exact Nov 10 2024 date); turn 182 is an ordinary user turn, not an instruction, so the instructionRules rule gained an optional `companionPattern` matched against user-source turns plus a `limit: 2`, and the new `instruction.nonProvisionalFilingDate` family pins both turns (gate on the "non-provisional patent filing" / "scheduled" question, instruction pattern on the "always confirm exact dates" turn, companion pattern on the "Non-Provisional Patent Filing Deadline: November 10, 2024" turn).
