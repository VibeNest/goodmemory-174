...
- Latest accepted retained diagnostic: `run-phase63-beam-100k-recall-diagnostic-rules-timeline-date-format-current-20260612T181900Z`. Compared with `run-phase63-beam-100k-recall-diagnostic-rules-immigration-consultant-session-current-20260612T180400Z`, it raises evidence-chat recall to 0.8313659736194947, lowers missed-recall cases to 93/355, wrong-recall/noise to 236/400, and zero-recall to 16, and moves global hit/missing/noise ids from 892/202/1258 to 893/201/1253. Target `10:instruction_following:1` goes from zero recall to full recall, returning the single designated standing-instruction turn 66 (the Month Day, Year date-format instruction) while removing all five noisy chats 274/297/6/214/228, with zero new noise and exactly one case delta. Repair mechanism note: the instruction turn previously arrived only via the instruction append-and-prune augmenter, so the repair follows the resumeDesign precedent — a dedicated early-return instruction route keyed on a timelineDateFormatInstructionQuery run-context flag, not the shared augmenter stand-down.
