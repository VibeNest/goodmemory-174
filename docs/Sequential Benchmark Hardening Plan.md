...
- Latest accepted retained diagnostic: `run-phase63-beam-100k-recall-diagnostic-rules-hiring-automation-topics-event-order-current-20260615T003720Z`. Compared with `run-phase63-beam-100k-recall-diagnostic-rules-mentor-interactions-event-order-current-20260615T001300Z`, it raises evidence-chat recall to 0.8928683210373352, lowers missed-recall cases to 68/355 and wrong-recall/noise to 211/400 (zero-recall stays 0), and moves global hit/missing/noise ids from 945/149/1006 to 947/147/999 — a 7-id net noise drop (global noise now under 1000). Target `11:event_ordering:2` ("order in which I brought up different cost-saving and involvement topics related to hiring automation ... five items") goes from recall 0.6 to full recall: it recovers the two missing designated turns 46/200 (44/104/198 were already hits) and sheds all 7 noisy chats with zero new noise. The repair adds a `hiringAutomationTopicsEventOrder` coverage selector (five facets — cost comparison turn 44, pilot results turn 104, long-term savings turn 198, training-involvement follow-up turn 46, expansion-involvement follow-up turn 200) chained in `sourceOrderTemporal.ts` with a `hiringAutomationTopicsEventOrderPlanActive` flag OR'd into `sourceOrderedNamedEntityEventPlanActive`. The two recovered turns are short follow-up questions the broad fallback could not rank. This question reads as an aggregate (cost-saving money cues plus a "five items" count), so the aggregate route otherwise preempted the coverage; the fix adds a one-gate `!isHiringAutomationTopicsEventOrderQuery(query)` guard to `aggregateEvidenceQuery` (affects only this 1/400 query). Cleanest possible pass: exactly one case delta, zero ripples. It is the ninth event-order coverage selector.
