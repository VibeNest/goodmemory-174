...
- Latest accepted retained diagnostic: `run-phase63-beam-100k-recall-diagnostic-rules-zoom-call-schedule-update-group-current-20260615T172117Z`. Second answer_ai_question knowledge_update recovery via the multi-facet matcher. Compared with the grocery-budget baseline (20260615T164144Z), it raises evidence-chat recall to 0.9540800357701766, lowers missed-recall cases to 25/355 and wrong-recall/noise to 172/400 (zero-recall stays 0), and moves global hit/missing/noise ids from 1012/82/825 to 1013/81/824. Target `8:knowledge_update:1` ("When is my Zoom call with the creative director scheduled?") goes from recall 0.667 to full recall: the original (April 21 3 PM, turn 92) and reschedule (April 22 11 AM, turn 94) were already hit, and this recovers the missing answer_ai_question schedule-confirmation turn 96 (a markerless schedule dump). Returns [92,94,96]. Added as one MULTI_FACET group (`knowledgeUpdate.zoomCallScheduleGroup` = "when is my Zoom call" && "scheduled", 1/400 — the narrowed gate avoids the same conversation's 8:msr:2/8:tr:1 which also mention the Zoom call); the schedule facet keys on "schedule for April 22" + "Team meeting" so it AVOIDS the disallowed fixture names (Laura, Michael) that appear in that turn's activity list — selector files disallowed-scan clean. Cleanest possible pass: exactly one case delta, zero ripples; all conv-8 cases held. IMPORTANT PROCESS NOTE: a prior attempt this loop to recover an instruction_following case (1:if:1) via multiFacetGroups was REVERTED — its query gate collided with the existing instructionRules continuation mechanism and broke 3 existing unit tests (the per-400 diagnostic did not catch it; the full suite did). So instruction_following needs the instructionRules companionPattern (pass-43) recipe, NOT multiFacetGroups; knowledge_update answer_ai_question cases have no such conflict. Remaining partial-recall families: knowledge_update (1 — 11:ku:2, answer_ai_question), instruction_following (6 — via instructionRules companionPattern, not multiFacetGroups), multi_session_reasoning (11, "how many" aggregates have an upstream candidate-pool recall gap). The prior accepted recall change was the first answer_ai_question recovery (16:knowledge_update:1, grocery-budget), which proved the multi-facet matcher reaches markerless turns and left recall at 0.9531410686340265.
