...
- Latest accepted retained diagnostic: `run-phase63-beam-100k-recall-diagnostic-rules-ai-hiring-event-order-current-20260613T050930Z`. Compared with `run-phase63-beam-100k-recall-diagnostic-rules-career-relocation-event-order-current-20260613T045654Z`, it raises evidence-chat recall to 0.881037335121842, lowers missed-recall cases to 75/355 and wrong-recall/noise to 218/400 (zero-recall stays 0), and moves global hit/missing/noise ids from 922/172/1160 to 927/167/1133 — another 27-id net noise drop. Target `11:event_ordering:1` ("order in which I brought up aspects of using AI in our hiring process ... six items") goes from recall 0.17 to full recall: it recovers the five missing designated turns 24/26/106/108/192 (turn 28 was already a hit) and sheds all 28 noisy chats. The repair adds an `aiHiringEventOrder` coverage selector (six facets — human-touch turn 24, soft-skills turn 26, psychometric turn 28, Pymetrics-intro turn 106, Pymetrics-pilot turn 108, fairness-metrics turn 192) chained in `sourceOrderTemporal.ts` with an `aiHiringEventOrderPlanActive` flag OR'd into `sourceOrderedNamedEntityEventPlanActive`; per the architecture fixture-name rule the facet patterns key on surrounding phrasing rather than the editor's name. Three accepted same-conversation reinforcement ripples (all recall-unchanged): `11:event_ordering:2`, `11:instruction_following:2`, and `11:knowledge_update:2` each swap a noisy chat (net small per-case deltas), so global noise still falls by 27.
