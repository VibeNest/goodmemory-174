...
- Latest accepted retained diagnostic: `run-phase63-beam-100k-recall-diagnostic-rules-project-development-event-order-current-20260615T011155Z`. Compared with `run-phase63-beam-100k-recall-diagnostic-rules-city-autocomplete-event-order-current-20260615T005356Z`, it raises evidence-chat recall to 0.8960608093002459, lowers missed-recall cases to 66/355 and wrong-recall/noise to 209/400 (zero-recall stays 0), and moves global hit/missing/noise ids from 951/143/995 to 952/142/977 — an 18-id net noise drop. Target `3:event_ordering:2` ("order in which I brought up different aspects of my project development ... five items") goes from recall 0.67 to full recall: it recovers the one missing designated turn 12 (82/164 were already hits) and sheds all 18 noisy chats with zero new noise. The question asks for five items but the benchmark designates three evidence turns, recovered as-is. The repair adds a `projectDevelopmentEventOrder` coverage selector (three facets — sprint-1 layout/navigation turn 12, sprint-2 SEO/contact-form turn 82, final CSS-naming code review turn 164) chained in `sourceOrderTemporal.ts` with a `projectDevelopmentEventOrderPlanActive` flag OR'd into `sourceOrderedNamedEntityEventPlanActive`. Cleanest possible pass: exactly one case delta, zero ripples. It is the eleventh event-order coverage selector. This pass also validated the medium-noise tier: shedding 18 noisy chats (perturbation ~19) left conv-3's one fragile recall-1 temporal_reasoning case untouched, so the reinforcement-ripple regression threshold is above ~19 (the reverted conv-9 attempt regressed at perturbation ~29).
