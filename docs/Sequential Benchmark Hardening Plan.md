...
- Latest accepted retained diagnostic: `run-phase63-beam-100k-recall-diagnostic-rules-estate-tax-rate-update-current-20260615T052857Z`. Sixth knowledge_update pass. Compared with `run-phase63-beam-100k-recall-diagnostic-rules-area-calculation-accuracy-update-current-20260615T051745Z`, it raises evidence-chat recall to 0.9186809747373128, lowers missed-recall cases to 50/355 and wrong-recall/noise to 195/400 (zero-recall stays 0), and moves global hit/missing/noise ids from 980/114/898 to 981/113/894. Target `19:knowledge_update:2` ("What is the estimated estate tax rate applied to assets above $200,000?", answer 12%) goes from recall 0.5 to full recall: it recovers the missing original turn 36 ("estate tax, which is 15% on assets above $200,000") to pair with the already-hit updated turn 56 ("the 12% rate on assets above $200,000"), and sheds its four noisy chats (241/204/41/130, two of which are assistant turns the user-only selector cleanly replaces). The repair adds an `estateTaxRate` update-series module (gate `updateSeries.estateTaxRate` requiring "estate tax rate" && "200,000" + ORIGINAL/UPDATED patterns + `selectSourceOrderedEstateTaxRateEvidence`) and one entry to the `UPDATE_SERIES_RULE_SELECTORS` table, modeled on the holiday-gift-budget module. SIGNIFICANT: conv-19 is the proven-poisoned conversation (the reverted pass 50 knocked `19:temporal_reasoning:1` from recall 1 to 0 via a 30-chat event_ordering reshuffle), yet this low-perturbation ku pair (shedding 4 noise) left BOTH conv-19 temporal_reasoning cases at recall 1 with zero ripple — establishing that the conv-19 poison is perturbation-magnitude-dependent (a high-noise reshuffle), not a conversation-wide hazard. Cleanest possible pass: exactly one case delta, zero ripples, zero newly-missing, zero new noise. Six knowledge_update cases now recovered; the clean source-ordered update-series workstream is now exhausted — the only remaining knowledge_update misses (`8/11/16:ku`) hinge on `answer_ai_question` turns that carry no `->->` source marker and so fall outside the recipe. NEXT PIVOT: the contradiction-pair recipe (three recovered, ten remain) once `contradiction.ts` (884/900) gets its own table extraction, or temporal_reasoning / multi_session_reasoning / instruction_following. The remaining partial-recall families are temporal_reasoning, multi_session_reasoning, knowledge_update, instruction_following, and contradiction_resolution.
