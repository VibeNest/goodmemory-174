...
- Latest accepted retained diagnostic: `run-phase63-beam-100k-recall-diagnostic-rules-legal-terms-explanation-instruction-current-20260613T041812Z`. Compared with the recall-neutral instructionRules-extraction baseline `run-phase63-beam-100k-recall-diagnostic-rules-instruction-rules-extraction-current-20260613T040036Z` (itself identical to the will-attorney-meeting-contradiction run), it raises evidence-chat recall to 0.8708025933378045, lowers missed-recall cases to 79/355, wrong-recall/noise to 222/400, and zero-recall to 2, and moves global hit/missing/noise ids from 914/180/1204 to 915/179/1198. Target `19:instruction_following:1` goes from zero recall to full recall, returning the single designated standing-instruction turn 62 while removing all six noisy chats 54/232/206/20/88/60, with zero new noise. There is one same-conversation reinforcement ripple: `19:multi_session_reasoning:1` stays at its prior recall with a net-zero noise swap (drops 88, adds 17), so global noise still falls by 6. Two infrastructure changes landed first: (a) sourceOrderInstruction.ts (895/900 lines) was refactored into an `instructionRules/` registry (one module per family + a dispatcher) so new instruction families add zero lines to the parent; (b) selection.ts now uses a single `instructionRuleFamilyQuery` run-context flag, and a matching narrow instruction gate suppresses `slotSpecificFactQuery` and the source-ordered-selector instruction suppression so the standing-instruction turn wins. The repair itself adds an `instruction.legalTermsExplanation` family (gate on the "my wishes are legally valid" question + the "always provide detailed explanations of legal terms when I ask about will requirements" turn pattern).
