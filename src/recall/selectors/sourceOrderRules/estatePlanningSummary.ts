...
const ESTATE_PROCESS_FACETS = [
  {
    ...
    role: "assistant",
    patterns: [
      /^(?=[\s\S]*\bestate\s+plan\b)(?=[\s\S]*\blist(?:ing)?\s+(?:all\s+)?(?:your\s+)?assets?\b)(?=[\s\S]*\bspecif(?:y|ying|ied)\s+provisions?\b)(?=[\s\S]*\bbeneficiary\s+designations?\b)/iu,
    ],
  },
  {
    ...
    role: "assistant",
    patterns: [
      /^(?=[\s\S]*\bchoos(?:e|ing)\b[\s\S]{0,80}\bexecutor\b)(?=[\s\S]*(?:\bApril\s+1\b|\blegal\s+and\s+financial\s+knowledge\b))(?=[\s\S]*\bresponsibility\b)/iu,
    ],
  },
  {
  ...
  },
  {
    ...
    role: "assistant",
    patterns: [
      /^(?=[\s\S]*\b\$?5,000\b)(?=[\s\S]*\bemergency\s+fund\b)(?=[\s\S]*\bguardianship\s+expenses?\b)(?=[\s\S]*(?:\bguardian\s+supporter\b|\bsame\s+page\b|\bproductive\s+conversation\b|\bseek\s+his\s+input\b))(?=[\s\S]*(?:\bmedical\s+costs?\b|\beducational\s+needs?\b|\bliving\s+expenses?\b))/iu,
    ],
  },
  {
    ...
    role: "assistant",
    patterns: [
      /^(?=[\s\S]*\bparalegal\b)(?=[\s\S]*\bwill\s+draft\b)(?=[\s\S]*\bguardianship\b)(?=[\s\S]*\basset\s+distribution\b)(?=[\s\S]*\bdigital\s+assets?\b)/iu,
    ],
  },
...
const WILL_FINALIZATION_FACETS = [
  {
    facet: "attorneyWitnessMeeting",
    role: "user",
    patterns: [
      /^(?=[\s\S]*\battorney\b)(?=[\s\S]*\bMarch\s+22\b)(?=[\s\S]*\bfinali[sz]e\s+my\s+will\b)(?=[\s\S]*(?:\bMontserrat\s+law\b|\btwo\s+witnesses\b))/iu,
    ],
  },
  {
    facet: "witnessReview",
    role: "assistant",
    patterns: [
      /^(?=[\s\S]*(?:\bwill\s+review\b|\breview\s+(?:your\s+)?will\b))(?=[\s\S]*\btwo[-\s]?witness\s+rule\b)(?=[\s\S]*\bnotari[sz](?:e|ed|ation)\b)(?=[\s\S]*\blegal\s+requirements?\b)/iu,
    ],
  },
  {
    ...
    role: "assistant",
    patterns: [
      /^(?=[\s\S]*\bnotari[sz]ed\s+affidavits?\b)(?=[\s\S]*\bguardianship\b)(?=[\s\S]*\bprobate\b)(?=[\s\S]*(?:\bbirth\s+certificates?\b|\bidentification\s+documents?\b))/iu,
    ],
  },
  {
  ...
  switch (facet) {
    ...
    case "guardianshipAffidavits":
      return normalizedContent.includes("notarized affidavits") &&
        normalizedContent.includes("guardianship");
    case "attorneyWitnessMeeting":
      return normalizedContent.includes("attorney") &&
        normalizedContent.includes("march 22");
    case "witnessReview":
      return normalizedContent.includes("review") &&
