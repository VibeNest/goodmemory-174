...
const FACET_PATTERNS = {
  julyDeadlineWorkshop: [
    /^(?=[\s\S]*\bJuly\s+25\s+workshop\b)(?=[\s\S]*\bJuly\s+22\s+project\s+deadline\b)(?=[\s\S]*\bGreg'?s\s+advice\b)(?=[\s\S]*\bstress\s+management\b)(?=[\s\S]*\bLeslie'?s\s+feedback\b)(?=[\s\S]*\bcommunication\s+skills\b)/iu,
  ],
  mockInterviewPrep: [
    /^(?=[\s\S]*\bmock\s+interview\b)(?=[\s\S]*\bGreg\b)(?=[\s\S]*\bApril\s+25\b)(?=[\s\S]*\bfollow-up\s+questions?\b)(?=[\s\S]*\bgood\s+impression\b)/iu,
  ],
  ninetyDayPlanDetails: [
  ...
  ninetyDayPlanReview: [
    /^(?=[\s\S]*\b90-day\s+plan\b)(?=[\s\S]*\bGreg\b)(?=[\s\S]*\bMay\s+30\b)(?=[\s\S]*\bobjectives\b)(?=[\s\S]*\bsuccess\s+metrics\b)/iu,
  ],
  portfolioUpdate: [
    /^(?=[\s\S]*\bportfolio\b)(?=[\s\S]*\bGreg\b)(?=[\s\S]*\bupdate\b)(?=[\s\S]*\bApril\s+1\b)(?=[\s\S]*\bstand\s+out\b)/iu,
  ],
