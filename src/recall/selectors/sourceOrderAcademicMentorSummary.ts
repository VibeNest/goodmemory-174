...
const QUERY_PATTERN =
  /^(?=[\s\S]*\bRobert\b)(?=[\s\S]*\b(?:summary|summarize)\b)(?=[\s\S]*\b(?:work|interactions?)\b)(?=[\s\S]*\bdeveloped\b)(?=[\s\S]*\b(?:key\s+steps|decisions?)\b)/iu;

const FACET_PATTERNS = {
  essayInfluence: [
    /^(?=[\s\S]*\bRobert'?s\s+1985\s+essay\b)(?=[\s\S]*\bgender\s+studies\b)(?=[\s\S]*\bApril\s+4\s+Zoom\s+call\b)(?=[\s\S]*\bargument\s+angles\b)(?=[\s\S]*\bnot\s+copying\b)/iu,
  ],
  finalProgressReview: [
  ...
  firstMentorMeeting: [
    /^(?=[\s\S]*\bacademic\s+mentor\b)(?=[\s\S]*\bRobert\b)(?=[\s\S]*\bretired\s+professor\b)(?=[\s\S]*\bEast\s+Janethaven\s+Library\b)(?=[\s\S]*\bFeb\s+10,\s+2024\b)(?=[\s\S]*\bgood\s+impression\b)/iu,
  ],
  journalConferenceDecision: [
    /^(?=[\s\S]*\bconference\s+paper\b)(?=[\s\S]*\bGreg\b)(?=[\s\S]*\bRobert\s+suggested\s+submitting\s+my\s+essay\s+to\s+a\s+journal\b)(?=[\s\S]*\bstronger,\s+more\s+persuasive\s+piece\b)/iu,
  ],
  warrantsFeedback: [
    /^(?=[\s\S]*\bRobert'?s\s+recommendation\b)(?=[\s\S]*\bstronger\s+warrants\b)(?=[\s\S]*\bgender\s+bias\b)(?=[\s\S]*\bMay\s+9\b)(?=[\s\S]*\bother\s+aspects\s+of\s+my\s+essay\b)/iu,
  ],
