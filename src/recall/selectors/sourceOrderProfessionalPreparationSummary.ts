...
const FACET_PATTERNS = {
  coverLetterFormat: [
    /\bsingle-column\s+format\b[\s\S]{0,180}\b(?:bold\s+headers?|mobile\s+reading|Laura)\b/iu,
    /\b(?:bold\s+headers?|mobile\s+reading|Laura)\b[\s\S]{0,180}\bsingle-column\s+format\b/iu,
  ],
  employeeHandbook: [
  ...
  storytellingInterview: [
    /\bstorytelling\b[\s\S]{0,220}\b(?:cultural\s+diversity|interview|Island\s+Media|Laura)\b/iu,
    /\b(?:cultural\s+diversity|interview|Island\s+Media|Laura)\b[\s\S]{0,220}\bstorytelling\b/iu,
  ],
  workshopPresentation: [
