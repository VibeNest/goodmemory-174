...
import {
  PHASE56_CANONICAL_GATE_RUN_ID,
  parsePhase56GateCliOptions,
  resolvePhase56CanonicalFallbackReportPath,
  resolvePhase56CanonicalLiveReportPath,
  resolvePhase56GateOutputDir,
  runPhase56Gate,
} from "../../scripts/run-phase-56-gate";
...
function buildLiveReport(input?: {
  ...
  return {
    benchmarkRoot: "/tmp/goodmemory/fixtures/implicitmembench-phase-56",
    generatedAt: "2026-05-02T00:00:00.000Z",
    generatedBy: "scripts/run-phase-56-live-memory.ts",
    kind: "goodmemory",
    manifestPath:
      "/tmp/goodmemory/fixtures/implicitmembench-phase-56/adapter-manifest.json",
    mode: "live",
    outputDir: "/tmp/goodmemory/reports/eval/live-memory/phase-56",
    profiles: {
    ...
    runDirectory: "/tmp/goodmemory/reports/eval/live-memory/phase-56/run-phase56-live-current",
    runId: "run-phase56-live-current",
    source: {
...
function buildSmokeReport(liveReport: Record<string, unknown>): Record<string, unknown> {
  return {
    ...liveReport,
    generatedBy: "scripts/run-phase-56-eval.ts",
    mode: "smoke",
    outputDir: "/tmp/goodmemory/reports/eval/fallback/phase-56",
    runDirectory:
      "/tmp/goodmemory/reports/eval/fallback/phase-56/run-phase56-fallback-current",
    runId: "run-phase56-fallback-current",
  };
}

describe("run-phase-56 gate", () => {
  it("resolves phase-56 gate output and canonical report paths", () => {
    expect(resolvePhase56GateOutputDir("/tmp/goodmemory")).toBe(
      "/tmp/goodmemory/reports/quality-gates/phase-56",
    );
    expect(resolvePhase56CanonicalFallbackReportPath("/tmp/goodmemory")).toBe(
      "/tmp/goodmemory/reports/eval/fallback/phase-56/run-phase56-fallback-current/report.json",
    );
    expect(resolvePhase56CanonicalLiveReportPath("/tmp/goodmemory")).toBe(
      "/tmp/goodmemory/reports/eval/live-memory/phase-56/run-phase56-live-current/report.json",
    );
  });

  it("parses phase-56 gate cli flags", () => {
    expect(
      parsePhase56GateCliOptions([
        "bun",
        "run",
        "scripts/run-phase-56-gate.ts",
        "--live-report-path",
        "/tmp/live.json",
        "--output-dir",
        "/tmp/out",
  ...
  it("accepts when the live report passes every targeted task file", async () => {
    ...
    const report = await runPhase56Gate(
      {
        liveReportPath: "/tmp/live.json",
        outputDir: "/tmp/goodmemory/reports/quality-gates/phase-56",
        runId: "run-phase56-gate-test",
      },
      {
    ...
    expect(
      writes.has(
        "/tmp/goodmemory/reports/quality-gates/phase-56/run-phase56-gate-test/phase-56-quality-gate.json",
      ),
    ).toBe(true);
  });
  ...
  it("uses the canonical phase-56 gate run id by default", async () => {
    const writes = new Map<string, string>();
    const liveReport = buildLiveReport();
    const smokeReport = buildSmokeReport(liveReport);

    await runPhase56Gate(
      {
        liveReportPath: "/tmp/live.json",
        outputDir: "/tmp/goodmemory/reports/quality-gates/phase-56",
      },
    ...
    expect(
      writes.has(
        `/tmp/goodmemory/reports/quality-gates/phase-56/${PHASE56_CANONICAL_GATE_RUN_ID}/phase-56-quality-gate.json`,
      ),
    ).toBe(true);
  });
});
