...
import {
  PHASE55_CANONICAL_GATE_RUN_ID,
  parsePhase55GateCliOptions,
  resolvePhase55CanonicalFallbackReportPath,
  resolvePhase55CanonicalLiveReportPath,
  resolvePhase55GateOutputDir,
  runPhase55Gate,
} from "../../scripts/run-phase-55-gate";
...
function buildLiveReport(input?: {
  ...
  return {
    benchmarkRoot: "/tmp/goodmemory/fixtures/implicitmembench-phase-55",
    generatedAt: "2026-05-02T00:00:00.000Z",
    generatedBy: "scripts/run-phase-55-live-memory.ts",
    kind: "goodmemory",
    manifestPath:
      "/tmp/goodmemory/fixtures/implicitmembench-phase-55/adapter-manifest.json",
    mode: "live",
    outputDir: "/tmp/goodmemory/reports/eval/live-memory/phase-55",
    profiles: {
      "goodmemory-raw-experience": {
        ...
        cases: rawCases,
        executionFailures: 0,
        explicitRecallLeakCount: input?.rawLeakCount ?? 0,
    ...
    runDirectory: "/tmp/goodmemory/reports/eval/live-memory/phase-55/run-phase55-live-current",
    runId: "run-phase55-live-current",
    source: {
    ...
    summary: {
      ...
      explicitRecallLeakCount:
        (input?.rawLeakCount ?? 0) + (input?.distilledLeakCount ?? 0),
      passedBlockingCases:
...
function buildSmokeReport(liveReport: Record<string, unknown>): Record<string, unknown> {
  return {
    ...liveReport,
    generatedBy: "scripts/run-phase-55-eval.ts",
    mode: "smoke",
    outputDir: "/tmp/goodmemory/reports/eval/fallback/phase-55",
    runDirectory:
      "/tmp/goodmemory/reports/eval/fallback/phase-55/run-phase55-fallback-current",
    runId: "run-phase55-fallback-current",
  };
}

describe("run-phase-55 gate", () => {
  it("resolves phase-55 gate output and canonical report paths", () => {
    expect(resolvePhase55GateOutputDir("/tmp/goodmemory")).toBe(
      "/tmp/goodmemory/reports/quality-gates/phase-55",
    );
    expect(resolvePhase55CanonicalFallbackReportPath("/tmp/goodmemory")).toBe(
      "/tmp/goodmemory/reports/eval/fallback/phase-55/run-phase55-fallback-current/report.json",
    );
    expect(resolvePhase55CanonicalLiveReportPath("/tmp/goodmemory")).toBe(
      "/tmp/goodmemory/reports/eval/live-memory/phase-55/run-phase55-live-current/report.json",
    );
  });

  it("parses phase-55 gate cli flags", () => {
    expect(
      parsePhase55GateCliOptions([
        "bun",
        "run",
        "scripts/run-phase-55-gate.ts",
        "--live-report-path",
        "/tmp/live.json",
        "--output-dir",
        "/tmp/out",
  ...
  it("accepts when the live report passes every targeted task file", async () => {
    ...
    const report = await runPhase55Gate(
      {
        liveReportPath: "/tmp/live.json",
        outputDir: "/tmp/goodmemory/reports/quality-gates/phase-55",
        runId: "run-phase55-gate-test",
      },
      {
    ...
    expect(report.evidence.liveMemoryReport.explicitRecallLeakDelta).toBe(0);
    ...
    expect(
      writes.has(
        "/tmp/goodmemory/reports/quality-gates/phase-55/run-phase55-gate-test/phase-55-quality-gate.json",
      ),
    ).toBe(true);
  });
  ...
  it("uses the canonical phase-55 gate run id by default", async () => {
    const writes = new Map<string, string>();
    const liveReport = buildLiveReport();
    const smokeReport = buildSmokeReport(liveReport);

    await runPhase55Gate(
      {
        liveReportPath: "/tmp/live.json",
        outputDir: "/tmp/goodmemory/reports/quality-gates/phase-55",
      },
    ...
    expect(
      writes.has(
        `/tmp/goodmemory/reports/quality-gates/phase-55/${PHASE55_CANONICAL_GATE_RUN_ID}/phase-55-quality-gate.json`,
      ),
    ).toBe(true);
  });
});
