...
|---|---|---:|---:|---|
| LongMemEval full 500 | judge-free 确定性子集回答准确率（按构造排除同模型 semantic judge） | **0.720**（360/500），`goodmemory-rules-only`；比无记忆基线高 65.2 个百分点；evidence-session recall 0.9543 | 无记忆基线 0.068（34/500，其中 30 个是纯弃答） | [longmemeval.json](./benchmark-claims/longmemeval.json) |
...
以下各行是研究与加固证据，不是声明。每一行都被它自己已提交的 declaration 挡在公开声明之外，
declaration 里记录了确切的 blocker；底层运行报告位于 gitignored 的 `reports/` 下，
可按 declaration 中记录的运行命令复现。
...
| BEAM（100K，rules-only 检索诊断） | recall **fitted 0.9621**（所有窄门开启；400 题中 355 道证据题）对 **generalization 0.6822**（151 个窄门全部关闭）；实测回答 checkpoint 278 / 400（0.695），对照引入 evidence pack 之前的 224 / 400（0.56），`executionFailures: 0` | 回答 checkpoint 用同模型判官；28 个百分点的 recall 差距是 scenario-fitted 贡献、不是通用检索（见 [ADR-005](./adr/ADR-005-scenario-fitted-recall-boundary.txt)）；generalization 低于 ≥0.75 目标 | [beam.json](./benchmark-claims/beam.json) |
| LoCoMo | 代表性 conv-1 live 运行 0.020（4/199）；词面/规则底座上 exact gold-turn recall 约 0.07-0.08、zero-retrieval 约 0.92 | 已入档的检索边界——回答被 recall 卡死；语义 candidate-generation 检索工作进行中 | [locomo.json](./benchmark-claims/locomo.json) |

按照 [ADR-005](./adr/ADR-005-scenario-fitted-recall-boundary.txt)，BEAM recall
以双指标报告：`fitted`（所有窄门开启）与 `generalization`（所有窄门关闭）。
fitted 数字的很大一部分来自为特定 BEAM case 调出来的 scenario-fitted query
classifier，它们在无关用户数据上不会触发；读者应把 generalization 数字当作
out-of-distribution 输入的下限。BEAM 回答 checkpoint（0.695，通用 answer-time
evidence pack `src/answer/evidencePack.ts`）此外还建立在该 fitted recall 之上、
并用回答模型给自己当判官，所以在 answer-gap 加固与独立判官评分成熟之前保持内部。
...
项目内 `npm install goodmemory@0.3.6` 不会把 `goodmemory` 放进 shell 的 `PATH`。
...
`npm install -g goodmemory@0.3.6` 安装的全局 CLI。本地 dependency install
